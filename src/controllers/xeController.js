...
const { getXeBydId, createXeService, removeCarService } = require("../services/xeService");
class xeController {
  ...
  async createXe(req, res) {
    try {
       const { tenXe, bienSoXe, nhaSanXuat, colorXe, loaiXe, sucChua } = req.body;
      const newXe = await createXeService({tenXe, bienSoXe, nhaSanXuat, colorXe, loaiXe, sucChua});
      if (!newXe) {
        return res.status(400).json({
          status: "false",
